<template>
  <div class="max-w-2xl mx-auto p-4">
    ...
    <!-- 教學彈窗按鈕 -->
    <button type="button" class="mb-4 px-3 py-1 bg-green-600 text-white rounded"
      @click="showGuide = true">如何新增音樂祭</button>
    ...
    <div class="mb-4">
      ...
      <div class="flex flex-wrap gap-2 mb-2">
        <span v-for="(c, idx) in festival.contributors" :key="idx"
          class="flex items-center bg-gray-200 rounded px-2 py-1 text-sm">
          <a :href="`https://github.com/${c}`" target="_blank" class="text-blue-600 hover:underline mr-1">{{ c }}</a>
          <button type="button" @click="festival.contributors.splice(idx, 1)"
            class="ml-1 text-xs text-red-500">✕</button>
        </span>
      </div>
      <div class="flex gap-2">
        <input v-model="newContributor" class="border rounded px-2 py-1 flex-1" placeholder="輸入 GitHub 帳號後按新增"
    ...
    <form @submit.prevent="handleExport" class="space-y-4">
      <div>
        ...
        <input v-model="festival.festivalId" class="border rounded px-2 py-1 w-full" required pattern="[a-z0-9\-]+"
          @input="validateId" />
      ...
      <div class="flex gap-2">
        <div class="flex-1">
          ...
          <input v-model.number="festival.location.latitude" type="number" step="any"
            class="border rounded px-2 py-1 w-full" />
        </div>
        <div class="flex-1">
          <label class="block font-bold mb-1">經度</label>
          <input v-model.number="festival.location.longitude" type="number" step="any"
            class="border rounded px-2 py-1 w-full" />
        </div>
      </div>
      <div>
      ...
      <div class="border-t pt-4 mt-4">
        <div class="flex items-center mb-2">
          ...
          <button type="button" @click="addStage"
            class="ml-2 px-2 py-1 bg-blue-500 text-white rounded text-xs">新增舞台</button>
        </div>
        <div v-for="(stage, sidx) in festival.stages" :key="sidx" class="border rounded p-2 mb-4">
          <div class="flex items-center mb-2">
            ...
            <button type="button" @click="removeStage(sidx)"
              class="ml-2 px-2 py-1 bg-red-500 text-white rounded text-xs">移除</button>
          </div>
          <div class="flex items-center mb-1">
            <span class="font-bold text-sm">表演</span>
            <button type="button" @click="addPerformance(sidx)"
              class="ml-2 px-2 py-1 bg-green-500 text-white rounded text-xs">新增表演</button>
          </div>
          <div v-for="(perf, pidx) in stage.performances" :key="pidx"
            class="border-2 border-gray-300 rounded p-2 mb-2 bg-gray-50">
            <div class="flex gap-2 mb-2">
              ...
              <button type="button" @click="removePerformance(sidx, pidx)"
                class="px-2 py-1 bg-red-400 text-white rounded text-xs">移除</button>
            </div>
            <div class="flex gap-2">
              <input v-model="perf.start" type="datetime-local" class="border rounded px-2 py-1 flex-1"
                placeholder="開始時間 (UTC+8)" />
              <input v-model="perf.end" type="datetime-local" class="border rounded px-2 py-1 flex-1"
                placeholder="結束時間 (UTC+8)" />
            </div>
          </div>
        </div>
      </div>
